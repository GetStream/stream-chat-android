...
internal class MessagingChannelLogic(
    ...
    override suspend fun onQueryChannelResult(request: QueryChannelRequest, result: Result<Channel>) {
        when (result) {
            is Result.Success -> channelStateLogic.propagateChannelQuery(result.value, request)
     ...
     * @param limit The limit of messages inside the channel that should be requested.
     */
    override suspend fun watch(userPresence: Boolean, limit: Int): Result<Channel> {
        logger.i { "[watch] userPresence: $userPresence, limit: $limit" }
     ...
     *
     * @param messageId Id of message before which to fetch messages. Last available message will be calculated if the
     * parameter is null.
