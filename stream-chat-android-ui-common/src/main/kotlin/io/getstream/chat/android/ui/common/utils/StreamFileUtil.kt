...
public object StreamFileUtil {
     ...
     * otherwise [Result.Failure] will contain a [StreamError].
     */
     ...
     * if the file was successfully fetched from the cache. Returns a [StreamError]
            ...
            when (val getOrCreateCacheDirResult = getOrCreateStreamCacheDir(context)) {
                ...
                is Result.Success -> {
                    ...
                    if (isFileCached) {
                        ...
                        Result.Failure(StreamError.GenericError(message = "No such file in cache."))
                    }
                }
            }
        } catch (e: Exception) {
                ...
                is Result.Success -> {
                    ...
                    if (file.exists() &&
                        ...
                        val fileUrl = attachment.assetUrl ?: attachment.url ?: return Result.Failure(
                            StreamError.GenericError(message = "File URL cannot be null.")
                        )
    ...
    private fun createFailureResultFromException(throwable: Throwable?): Result.Failure {
        return Result.Failure(
            throwable?.let { exception ->
                StreamError.ThrowableError(message = "Could not write to file.", cause = exception)
            } ?: StreamError.GenericError(message = "Could not write to file.")
        )
    }
