...
public object MediaStringUtil {
    ...
    @JvmStatic
    public fun convertVideoLength(videoLengthInSeconds: Long, locale: Locale = Locale.getDefault()): String = if (videoLengthInSeconds <= 0) {
        "--:--:--"
    } else {
    ...
    @JvmStatic
    public fun convertFileSizeByteCount(bytes: Long): String = when {
        bytes <= 0 -> "0 B"
