...
internal object MessageListItemDiffCallback : DiffUtil.ItemCallback<MessageListItem>() {
    override fun areItemsTheSame(oldItem: MessageListItem, newItem: MessageListItem): Boolean = oldItem.getStableId() == newItem.getStableId()

    override fun areContentsTheSame(oldItem: MessageListItem, newItem: MessageListItem): Boolean = when (oldItem) {
    ...
    ): MessageListItemPayloadDiff? = (oldItem as? MessageListItem.MessageItem)?.let { oldMessageItem ->
    ...
    private fun getMessageChangePayload(
        ...
        return MessageListItemPayloadDiff(
            ...
            isMessageRead = oldItem.isMessageRead != newItem.isMessageRead,
        )
    }

    override fun getChangePayload(oldItem: MessageListItem, newItem: MessageListItem): Any? = getMessageListItemChangePayload(oldItem, newItem)
}
