...
internal fun String.applyTypeface(typeface: Int, ranges: List<IntRange>): SpannableString = SpannableString(this).apply {
    ranges.forEach {
        setSpan(StyleSpan(typeface), it.first, it.last + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    }
}

internal fun String.bold(items: List<String>? = null, ignoreCase: Boolean = false): SpannableString = applyTypeface(BOLD, getOccurrenceRanges(items, ignoreCase))

internal fun String.italicize(items: List<String>? = null, ignoreCase: Boolean = false): SpannableString = applyTypeface(ITALIC, getOccurrenceRanges(items, ignoreCase))
