...
internal class MessageRepositoryTest : BaseDomainTest() {
    ...
    fun testSelectMessagesForChannel() = runBlocking {
        ...
        var messages = repo.selectMessagesForChannel(data.message1.cid, pagination) { data.userMap[it]!! }
        ...
        messages = repo.selectMessagesForChannel(data.message1.cid, pagination) { data.userMap[it]!! }
        Truth.assertThat(messages.size).isEqualTo(1)
