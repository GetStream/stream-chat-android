...
/**
 * Returns [LogicRegistry] instance that handles OfflinePlugin logic.
 */
@ExperimentalStreamChatApi
internal val ChatClient.logic: LogicRegistry
    get() = requireNotNull((plugins.firstOrNull { it.name == OfflinePlugin.MODULE_NAME } as? OfflinePlugin)?.logic) {
        "Offline plugin must be configured in ChatClient"
    }
