...
public class QuerySortParser<T : ComparableFieldProvider> {
    ...
    ): QuerySorter<T> = specs.fold(QuerySortByField()) { sort, sortSpecMap ->
     ...
     */
    public fun fromSpecifications(specs: List<SortSpecification<T>>): QuerySorter<T> = specs.fold(QuerySortByField()) { querySort, sortSpec ->
    ...
    private fun createQuerySort(
        ...
        fieldName: String,
        direction: Int,
    ): QuerySortByField<T>? = when (direction) {
