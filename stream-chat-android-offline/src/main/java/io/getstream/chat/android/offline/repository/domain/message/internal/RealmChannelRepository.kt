...
public class RealmChannelRepository(private val realm: Realm) : ChannelRepository {
  ...
  override suspend fun deleteChannel(cid: String) {
    realm.writeBlocking {
      realm.query<ChannelEntityRealm>("channel_id == \'$cid\'")
        .first()
        .find()
        ?.let(this::delete)
    }
  ...
  override suspend fun selectAllCids(): List<String> =
    realm.query<ChannelEntityRealm>().find().map { entity -> toCid(entity.type, entity.channel_id) }
  ...
  override suspend fun selectChannelCidsBySyncNeeded(limit: Int): List<String> =
    realm.query<ChannelEntityRealm>("sync_status == $0", SyncStatus.SYNC_NEEDED.status)
      ...
      .find()
      .map { entity -> toCid(entity.type, entity.channel_id) }
  ...
  override suspend fun selectChannels(
    ...
    val query = "channel_id IN $channelsString"
//    Log.d(LOG_TAG, "query: $query")

    return realm.query<ChannelEntityRealm>()
      .find()
      .map { entity -> entity.toDomain() }
      .also { channels ->
        Log.d(LOG_TAG, "found channels in database: ${channels.names()}")
//        logAllChannelsInDatabase()
      }
  }
  ...
  private suspend fun logAllChannelsInDatabase() {
    Log.d(LOG_TAG, "Those are all ids in database: ${selectAllCids()}")
  }
}

private fun toCid(type: String, id: String) = "$type:$id"
