...
internal class DatabaseQueryChannelsRepository(
     ...
     */
    override suspend fun selectBy(filter: FilterObject, querySort: QuerySorter<Channel>): QueryChannelsSpec? = queryChannelsDao.select(generateId(filter, querySort))?.let(Companion::toModel)
    ...
    private companion object {
        private fun generateId(filter: FilterObject, querySort: QuerySorter<Channel>): String = "${filter.hashCode()}-${querySort.toDto().hashCode()}"

        private fun toEntity(queryChannelsSpec: QueryChannelsSpec): QueryChannelsEntity = QueryChannelsEntity(
        ...
        private fun toModel(queryChannelsEntity: QueryChannelsEntity): QueryChannelsSpec = QueryChannelsSpec(
