...
Next, we query the chat client for channels in the `init` block. We do this by defining our request and passing it to `chatClient.queryChannelsAsState`. We get back a `StateFlow<QueryChannelsState?>`, which we store in the `queryChannelsStateFlow` class property.
