...
* `reactionTypes`: Allows you customize which reactions show in the overlay. By default it uses `ChatTheme.reactionTypes` that is exposed through the [`ChatTheme`](../03-general-customization/01-chat-theme.mdx) component.

If you want to customize these options or reactions, you can set up custom reactions, using the [`ChatTheme`](../03-general-customization/01-chat-theme.mdx)'s `reactionTypes` parameter.
...
![Stylised SelectedMessageMenu component with a custom header content](../../assets/compose_selected_message_menu_custom_header_slot.png)
