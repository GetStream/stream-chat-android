...
:::note
`MessagesScreen` will give you the ability to display `ReactionsPicker` only if you have defined more than 5 reactions inside of [`ChatTheme`](../03-general-customization/01-chat-theme.mdx).
:::
...
* `reactionTypes`: Allows you customize which reactions are shown. By default it uses `reactionTypes` inside of [`ChatTheme`](../03-general-customization/01-chat-theme.mdx).
