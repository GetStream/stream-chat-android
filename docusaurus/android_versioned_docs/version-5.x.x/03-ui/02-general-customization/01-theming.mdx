...
You can find the full list of available attributes for each view linked on their individual component pages, or in the `attrs_view_name` files [here](https://github.com/GetStream/stream-chat-android/tree/v5/stream-chat-android-ui-components/src/main/res/values).
...
SDK contains following activities: `AttachmentMediaActivity`, `AttachmentActivity` and `AttachmentGalleryActivity`. You can customize them by overriding the activity with custom `theme` in your manifest:
...
Our SDK already provide a DayNight theme. If you want to force Dark or Light mode, you need to follow the default Android mechanism to use it:
