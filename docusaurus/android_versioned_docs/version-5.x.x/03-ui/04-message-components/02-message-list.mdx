...
//TODO - Edit this section after discussing with the team which ones we should list here
...
:::note
Handlers must be set before passing any data to `MessageListView`. If you don't use the default binding provided by `bindView`, please make sure to set up all the handlers yourself.
:::
...
Additionally, you can also use `ChatUI.markdown` for Markdown support. If you do that, don't use `android:autoLink` attribute because it'll break the markdown [Linkify](https://noties.io/Markwon/docs/v4/linkify/) implementation. This property is deprecated and `ChatUI.messageTextTransformer` should be used instead. See [docs](../02-general-customization/02-chatui.mdx) for more information.
