...
* `showDateSeparatorInEmptyThread`: Allows showing the DateSeparator item in the MessageListView, when opening an empty thread. This helps implement more complex use cases for empty threads and `ThreadPlaceholderItem`s.
...
### Showing DateSeparator item in empty threads

Overriding the `showDateSeparatorInEmptyThread` allows you to show the separator in empty threads:

<Tabs>
<TabItem value="kotlin" label="Kotlin">

```kotlin
ChatUI.showDateSeparatorInEmptyThread = true
```
</TabItem>
...
```java
ChatUI.setShowDateSeparatorInEmptyThread(true);
```
</TabItem>
</Tabs>
...
On its own, it might not look like much, but when customizing the `ThreadPlaceholderItem`, you can achieve powerful expanded behavior of threads, where you denote to the user when the parent message was created and that they're in an empty thread where they can leave replies.
