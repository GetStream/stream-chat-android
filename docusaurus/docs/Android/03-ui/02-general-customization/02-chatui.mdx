...
* `navigator`: Allows intercepting and modifying default navigation between SDKs components (for example redirection from `MessageListView` to `AttachmentGalleryActivity`).
...
You can override the default set of reactions. In order to define a custom set of reactions for your app, you need to pass the following data to the `ChatUI.supportedReactions` property:
