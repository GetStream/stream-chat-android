...
* `messageTextTransformer`: Allows customising the message text's format or style. For example, it can be used to provide markdown support in chat or it can be used to highlight specific messages by making them bold etc.
...
You can easily provide a transformer that can transform and apply the message text to given TextView. You need to override `ChatUI.messageTextTransformer` to an instance of `ChatMessageTextTransformer`s implementation.
...
If you want to use any other library or your own markdown implementation, it can be done like this:
