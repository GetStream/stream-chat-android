...
### Customizing Avatar
The SDK uses `AvatarView` in the lists of channels, users and in the message lists. The avatar is a small picture that identifies an specific channel or user. 

For `Channel`, it is used the property Channel.image and for User, it is used `User.image`. The image in the AvatarView, will be the image's url in those fields: 

![Default avatar](../assets/default_channel.png)

If there's no url to be displayed, the SDK falls back to a gradient color with initials for the channel or user: 
...
This way use default avatar with gradient colors and initials for the users.
