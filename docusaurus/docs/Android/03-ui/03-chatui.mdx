...
You can configure avatar shape, border width, online indicator and other aspects using [AvatarStyle](https://github.com/GetStream/stream-chat-android/blob/main/stream-chat-android/src/main/java/com/getstream/sdk/chat/view/messages/AvatarStyle.kt). Changing the shape and border radius, it is possible to create this kind of avatar: 

![Light_mode](../assets/square_avatar.png)

It is also possible to change the background and the picture using `AvatarBitmapFactory`.

#### Customizing Avatar AvatarBitmapFactory
...
**If you don't want to use avatars from internet and only use the default avatar**, you can call `createDefaultUserBitmap` inside `createUserBitmap` in your custom implementation of `AvatarBitmapFactory`
...
**If you only would like to change the gradient colors for the default avatar**, you can use `stream_ui_avatar_gradient_colors`. You can override this color in your color xml file like: 
