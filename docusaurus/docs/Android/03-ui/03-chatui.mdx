...
* `messageTextTransformer`: Allows customising the message text's format or style. For example, you can use it to provide markdown support or highlight specific messages by making them bold.
...
Stream UI TextView components doesn't have `android:autoLink` property set because it conflicts with Markdown plugins.
If you want to use a custom `ChatMessageTextTransformer` with text autolink, you can use `AutoLinkableTextTransformer`. After applying the transformation, it also makes the links clickable.
