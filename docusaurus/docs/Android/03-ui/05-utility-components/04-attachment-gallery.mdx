...
Some of these styles, such as `streamUiAttachmentGalleryTitleStyle` are applied to stock Android components, in this case a `TextView`. These accept any styled attribute that is normally applicable to the stock Android component.

<!-- TODO: Finish the list -->

There are others however, which apply custom styling attributes to custom Stream UI components, so let's explore them together:
