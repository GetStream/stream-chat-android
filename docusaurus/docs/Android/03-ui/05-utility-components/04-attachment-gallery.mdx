...
The gallery can be customized through the use of styleable attributes.

Let's start customizing our screen by styling the play button displayed over videos so that it features a flat design.

1. Create a style that changes the way the play button is displayed in the main viewing area of the gallery:
...
2. Create a style that changes the way the play button is displayed in the overview:
...
4. Finally, add the custom theme to your attachment gallery Activity theme, along with a few lines that make the status and navigation bars look prettier:
...
As you can see, we've replaced the elevated white circular play button with a flat black squircle.
...
### Exploring the Available Styling options

As you might have noticed, styling the attachment gallery is based on individual styles which are responsible for certain gallery features.

You can find all of the styles [here](https://github.com/GetStream/stream-chat-android/blob/develop/stream-chat-android-ui-components/src/main/res/values/attrs_attachment_gallery_activity.xml).<br/>
Some of these styles, such as `streamUiAttachmentGalleryTitleStyle` are applied to stock Android components, in this case a `TextView`. These accept any styled attribute that is normally applicable to the stock Android component. Others however power Stream UI components and have custom made styled attributes.kotlin
...
- `streamUiAttachmentGalleryOptionsStyle`: Styles the actions menu. Accepts custom Stream attributes, all of which are listed [here](https://github.com/GetStream/stream-chat-android/blob/develop/stream-chat-android-ui-components/src/main/res/values/attrs_attachment_options_view.xml).
- `streamUiAttachmentGalleryVideoAttachmentsStyle`: Styles how video attachments are displayed in the main viewing area of the gallery. Accepts custom Stream attributes, all of which are listed [here](https://github.com/GetStream/stream-chat-android/blob/develop/stream-chat-android-ui-components/src/main/res/values/attrs_attachment_gallery_video_attachments.xml).
- `streamUiMediaAttachmentGridViewStyle`: Styles how the media overview menu is displayed. Accepts custom Stream attributes, all of which are listed [here](https://github.com/GetStream/stream-chat-android/blob/develop/stream-chat-android-ui-components/src/main/res/values/attrs_media_attachment_grid_view.xml).
