...
The two methods that need to be implemented are similar to one from `AttachmentFactory`:
...
First we create the view that we are going to render for the attachment, call it `view_quoted_date_attachment`:
...
```
As before we are fetching the custom data from `attachment.extraData["payload"]`. This time the year will not be shown if it is the current one and the month will be in a new line.

After we have the view we wish to render for our custom component our `QuotedAttachmentFactory` will look like this:
...
To complete the quoted attachment feature and show it on UI you need to provide the `QuotedDateAttachmentFactory` via `ChatUI` alongside the `DefaultQuotedAttachmentFactory` to show the rest of the attachments:
...
Quoted message with date attachment should now be rendered inside the message list like on the screenshot below:
