...
Attachments are rendered inside various Chat UI Components such as the message list or the message composer. Different components allow you to stylize attachment rendering in different ways.
...
:::note
Only certain attributes were used here, you can find the rest in the [source file](https://github.com/GetStream/stream-chat-android/blob/develop/stream-chat-android-ui-components/src/main/res/values/attrs_media_attachment_view.xml).
:::
...
And finally, add that to your app theme:
...
Afterwards, add it to your custom attachment gallery `Activity` theme:
