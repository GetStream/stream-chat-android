...
### Transliteration
You can add transliteration to `DefaultUserLookupHandler` by setting the desired id for transliteration. This class only supports transliteration prior to API 29 (Android Q) and it will not apply transliteration for APIs smaller than this. 
...
You can also use your own implementation of `StreamTransliterator` to add support for it. An interesting library is [ICU4J](https://unicode-org.github.io/icu/userguide/icu4j)
