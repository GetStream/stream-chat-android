...
The recommended way of setting up `MessageComposerView` is by binding it to
a `MessageComposerViewModel`. This will make it fully functional by setting up any necessary
listeners and data handling.
...
Because it doesn't make sense to use the `MessageComposerView` as a standalone component, you'll
also need to integrate it with the `MessageListView`:
...
In the snippet above, you initialized the composer and integrated it with the `MessageListView` by
passing the actions from the message list to it.

## Handling Actions

To handle actions supported by the `MessageComposerView` you have to set the corresponding
listeners:
...
If you don't override the listeners, the default listeners from
the `MessageComposerViewModel::bindView` method will be used:
...
You can refer to the default implementations above when setting your own custom listeners.
...
* **Leading content**: The content shown at the start of part with the integration buttons.
* **Center content**:  The center part with the text input.
* **Trailing content**: The right part with the send button.
* **Header content**: The top part with the message mode.
* **Footer content**: The bottom part with the "also send to channel" checkbox.
* **Command suggestions content**: The content
