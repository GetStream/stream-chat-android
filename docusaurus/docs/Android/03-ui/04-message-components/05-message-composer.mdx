...
The recommended way of setting up `MessageComposerView` is by binding it to a `MessageComposerViewModel`. This will make it fully functional by setting up any necessary listeners and data handling.
...
In the snippet above, you initialized the composer and integrated it with the `MessageListView` by passing actions from the message list to it.
...
With the new `MessageComposerView` you can replace certain parts of the layout with custom content views.
...
* **Header content**: Represents the top part action mode title.
...
* **Mention suggestions content**: Represents the context inside the mention suggestions popup.
