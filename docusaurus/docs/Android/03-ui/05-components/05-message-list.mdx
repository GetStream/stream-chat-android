...
setUserReactionClickListener{ message: Message, user: User, reaction: Reaction ->
...
Additionally, you can also use `ChatUI.markdown` for Markdown support. If you do that, don't use `android:autoLink` attribute because it'll break the markdown [Linkify](https://noties.io/Markwon/docs/v4/linkify/) implementation. This property is deprecated and `ChatUI.messageTextTransformer` should be used instead. See [docs](../03-chatui.mdx) for more information.
...
```
## Configure giphy
It is possible to configure the gifs size. Prior of version 4.23.0 gifs are autosizable and will resize thenselves accordingly with the size of the image they load. It is possible to use constant size gifs using `streamUiMediaAttachmentConstantSize` and set the desired height using `streamUiMediaAttachmentGiphyHeight`.

As of v5.x.x, our Giphy holders are much more customizable. You can use three Giphy load modes:
* `original`: The original Giphy size, as given by the API.
* `fixedHeight`: Smaller size Giphy, that uses fixed height in order to improve scrolling performance and image loading speed.
* `fixedHeightDownsampled`: Same as `fixedHeight`, but downsampled to only 6 frames, making it 6x smaller and faster to load, but less visually appealing.

Alongside these Giphy types, you can also choose the `scaleType` when loading the Giphy:
* `fitCenter`: The recommended setting, fits the GIF to the center of the container and keeps aspect ratio.
* `centerCrop`: Useful if you want larger size giphies and items, but don't care if the image is cropped.

The recommended settings are using `fixedHeight` in pair with `fitCenter` to achieve the best performance and looks.