...
:::note
All components from the SDK have to be wrapped in [`ChatTheme`](general-customization/chat-theme.mdx) which provides theming and other customization options. You don't have to individually wrap components in [`ChatTheme`](general-customization/chat-theme.mdx), but it has to be present somewhere above the Chat components in the hierarchy.
:::
...
* [`ChannelsScreen`](channel-components/channels-screen.mdx): Builds a screen that shows a header with user information, the list of channels for the current user and a search input to filter those channels by name.
* [`MessagesScreen`](message-components/messages-screen.mdx): Builds a full chat experience with a header that shows information about the channel, list that supports messages (including attachments, reactions, replies, threads and more) and a message composer that lets you write messages and send attachments.
...
You can learn more about [`ChannelsScreen`](channel-components/channels-screen.mdx) and [`MessagesScreen`](message-components/messages-screen.mdx) on their respective pages.
