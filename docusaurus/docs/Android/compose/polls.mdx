...
If you see the code above the `PickerTabContent` composable function consists of several compoents, such as `PollCreationHeader`, `PollQuestionInput`, `PollOptionList`, `PollSwitchList`, and `PollCreationDiscardDialog`. Let's break down each UI component one by one.
...
The reorderable list of options consists of input fields where each entry must be unique. If a duplicate entry is detected, an error message will be displayed. This ensures that all poll options are distinct, maintaining the integrity and clarity of the poll.
