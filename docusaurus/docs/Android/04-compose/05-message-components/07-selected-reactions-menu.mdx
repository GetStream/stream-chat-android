...
The component shows reactions from the [`ChatTheme`](../03-general-customization/01-chat-theme.mdx) on the top, followed by a list of message reactions.
...
* `reactionTypes`: Allows you to customize reactions shown in the overlay. By default it uses `ChatTheme.reactionTypes` which is exposed by the [`ChatTheme`](../03-general-customization/01-chat-theme.mdx) component.

If you want to customize the reactions, you can set up custom reactions, using [`ChatTheme`](../03-general-customization/01-chat-theme.mdx)'s `reactionTypes` parameter.
