...
You can also provide your own instance of `MessagesLazyListState` based on your custom logic where you define the starting scroll position of the state, the offset and how it behaves based on user events. For example, when deep linking or calculating offset when scrolling to message.
