...
## Previewing Image and Video Attachments

Image and video attachments are treated differently compared to file attachments and are previewed as thumbnails which can be displayed as a single tile or multiple ones depending on how many attachments are contained within the specific message. By default, these are rendered by `MediaAttachmentFactory`.

:::note
Attachment factories are used for previewing attachments. You can use those we provide out of the box, add to them, or replace them with your own. You can read more about them [here](../03-general-customization/01-chat-theme.mdx#streamattachmentfactories).
:::
...
If you want to read more about how to customize the way these attachments are displayed, head over to the relevant section seen [here](#customizing-the-way-image-and-video-attachments-are-previewed)
...
### Customizing the Way Image and Video Attachments are Previewed

If we take a look at the function signature of `MediaAttachmentFactory`, we see the following
...
* `maximumNumberOfPreviewedItems`: Controls the maximum amount of tiles that can be displayed when previewing attachments inside the message list. In case this value is smaller than the number of attachments contained in the message, a count informing us how many more are left will be displayed.
...
* `previewItemOverlayContent`: Serves the same function as `itemOverlayContent`, only used inside `MessageComposer` when selecting attachments.
