...
* `predicate`: Lambda function that accepts a list of attachments in a message and returns `true` if a given factory can consume the attachments and show some UI.
...
Each of these factories supplies a predicate, as well as a content composable lambda function that calls `LinkAttachmentContent`, `ImageAttachmentContent` or `FileAttachmentContent`.
