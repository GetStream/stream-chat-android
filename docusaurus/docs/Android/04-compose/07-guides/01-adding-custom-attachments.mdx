...
Stream SDK supports quoting the messages which are shown inside the regular message content. Being nested inside a message they heave less space available and require different layouts and with that factories.

If you don't wish or have no need to customize the layout there is no need since the factories are of the same type and can be reused. If there is no `quotedAttachmentFactory` that can render that type of attachment we default to `attachmentFactory` in our default components to one that can if there is one.
...
As before we are fetching the custom attachment data using `attachment.extraData["payload"]`. Difference here is we hide the year if it is the current one and newline the date for a more compact preview.

This time we don't need the `textFormatter` or `previewContent`. The `textFormatter` is used to show the description of the attachment in the `ChannelList` and that will be handled by the parent message. `previewContent` is used in the `MessageComposer` and it only shows the parent message.
...
Quoted date attachments should now be correctly rendered in the quoted message in the message list like on the screenshot below:

![Rendering Custom Attachment](../../assets/compose_custom_quoted_message_attachment_factory.png)

With even less lines of code than the previous custom attachment factory we have adjusted the date preview to fit a smaller layout retaining all the relevant data. This gives you even more fine grained customization to build powerful and responsive UI with the same reusable components.
