...
As before, we are fetching the custom attachment data using `attachment.extraData["payload"]`. We also split the full date into two lines for a more compact preview.
