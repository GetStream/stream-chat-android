...
:::note
Video Attachment preview thumbnails are a paid feature and as such can be turned off. You can find the pricing [here](https://getstream.io/chat/pricing/) and the information on how to turn them off in the [this](../05-message-components/03-message-list.mdx#image-and-video) section.
:::
...
* `maximumNumberOfPreviewedItems`: Controls the maximum amount of tiles that can be displayed when previewing attachments inside the message list. In case this value is smaller than the number of attachments contained in the message, a count informing us how many more are left is displayed.
...
Remember, order of precedence matters when creating a list of attachment factories, as the first one whose `canHandle()` function evaluates to true will render the given attachment.
