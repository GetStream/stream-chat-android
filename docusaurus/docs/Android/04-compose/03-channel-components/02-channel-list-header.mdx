...
:::note
The `trailingContent` parameter uses the `onHeaderActionClick` parameter for the default header action. If you want to keep the same UI but override the behavior, you need to change `onHeaderActionClick`. Otherwise, skip over to **Customization** to learn how to customize the action UI.
:::
...
* `modifier`: Modifier for the root component styling. You can add things like background, elevation, padding, and more.
...
* `centerContent`: Customizable composable function parameter that overrides the center content. This allows you to either:
    ...
    * or override both the UI and behavior by changing the `centerContent` parameter from the default to your own.
...
<!-- TODO WIP PAGE You can also choose to build a completely custom header and integrate it with the rest of our components. To learn how to do that, read our [Building Custom Screens](../_07-guides/06-building-custom-screens.mdx) guide. -->
