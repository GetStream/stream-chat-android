...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContent {
        ...
        ChatTheme {
            ChannelList(
                itemContent = { channel -> // Customize the channel items
                    DefaultChannelItem(
                        channel = channel,
                        currentUser = user,
                        onChannelLongClick = { },
            ...
            ChannelList(
                itemContent = { channel ->
                    DefaultChannelItem(
                        channel = channel,
                        currentUser = user,
                        onChannelLongClick = { viewModel.selectChannel(it) },
...
override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContent {
        ...
        ChatTheme {
            ChannelList(
                itemContent = { channel -> // Customize the channel items
                    CustomChannelListItem(channel = channel, user = user)
                }
            )
        }
    }
...
fun CustomChannelListItem(channel: Channel, user: User?) {
    DefaultChannelItem(
        ...
        trailingContent = { // Replace the last message content with a spacer
...
As you can see, it's very easy to override and completely replace the Slot APIs in our `DefaultChnnelItem`. In the example, you replaced the `trailingContent` with a simple `Spacer` for some padding and the `detailsContent` with a `Text` that shows the channel name.
