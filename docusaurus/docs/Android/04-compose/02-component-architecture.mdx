...
:::note
All components from the SDK have to be wrapped in a [`ChatTheme`](02-general-customization/01-chat-theme.mdx) which provides theming and other customization options. You don't have to individually wrap components in `ChatTheme`, but it has to be present somewhere above the Chat components in the hierarchy.
:::
...
* [`ChannelsScreen`](./03-channel-components/01-channels-screen.mdx): Builds a screen that shows a header with user information, the list of channels for the current user and a search input to filter those channels by name.
...
You can learn more about [`ChannelsScreen`](./03-channel-components/01-channels-screen.mdx) and [`MessagesScreen`](./04-message-components/01-messages-screen.mdx) on their respective pages.
