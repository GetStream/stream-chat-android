...
* `onScrolledToBottom`: Handler for the user reaching the start of the list (newest message). Used to remove the "New message" or "Scroll to bottom" actions from the UI.
...
* `itemContent`: Composable function that allows you to fully override the UI and behavior of each message in the list. This function will be applied to each item in the list and you'll gain access to the `MessageListItem` inside the lambda when building your custom UI.
...
fun CustomMessageList() {
    MessageList(
        ...
        itemContent = { item ->
            if (item is MessageItem) { // we check that this is not a `DateSeparator` item
