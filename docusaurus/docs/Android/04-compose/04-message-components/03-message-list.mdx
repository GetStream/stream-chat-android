...
MessageList(
    ...
    lazyListState = if (parentMessageId != null) rememberLazyListState() else myListState,
)
```

We recommend keeping a separate instance of the `LazyListState` when you're in a thread, as per the condition above. This helps you keep the scroll state of the main list intact when you enter and leave threads.

You can also provide custom logic where you define the starting scroll position of the state based on custom logic. For example, when deep linking.
