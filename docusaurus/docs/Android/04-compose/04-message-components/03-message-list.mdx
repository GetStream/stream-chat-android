...
* **Stateless**: This is a stateless version of the list which doesn't know about the `ViewModel` and depends on pure state from external sources in order to render its UI.
...
* `onReactionsClick`: Handler used when the user taps on a reaction left on a message.
* `onMessagesStartReached`: Handler for the user reaching the oldest message, to trigger pagination.
* `onLastVisibleMessageChanged`: Handler when the user scrolls and the last visible item changes.
* `onScrollToBottom`: Handler for the user reaching the newest message. Used to remove the "New message" or "Scroll to bottom" actions from the UI.
...
* `onImagePreviewResult`: Handler when the user receives a result from the Image Preview screen, after opening an image attachment.
...
* `loadingContent`: Composable function used while loading the message data. By default shows a loading indicator that you can override.
* `emptyContent`: Composable function called when loading is finished and no messages are available. By default displays a message that you can override.
* `itemContent`: Composable function that allows you to fully override the UI and behavior of each message in the list. This function will be applied to each item in the list and you'll gain access to the `MessageListItemState` inside the lambda when building your custom UI.
