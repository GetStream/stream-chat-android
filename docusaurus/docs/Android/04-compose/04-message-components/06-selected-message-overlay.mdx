...
As you can see, adding the menu is very simple. If a `selectedMessage` exists, you pass in the message options you want to expose to the user, as well as the selected message. The reactions you show are taken from the [`ChatTheme`](../02-general-customization/01-chat-theme.mdx) component and everything else required to show the component is taken care of internally.
...
* `bodyContent`: Allows you to customize what content you show on the bottom part of the menu. By default it shows `MessageOptions`
