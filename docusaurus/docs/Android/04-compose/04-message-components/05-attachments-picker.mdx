...
Handler when the user reaches the last item i

* `onAttachmentsSelected`: Handler when the user selects and confirms attachments by clicking on the **Confirm selection** button.
* `onDismiss`: Handler when the picker is dismissed by clicking outside of the picker area or pressing the system back button.
