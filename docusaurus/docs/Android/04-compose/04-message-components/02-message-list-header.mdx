...
This component is stateless and doesn't have it's own `ViewModel`. You need to provide the data to the header, otherwise it doesn't know what to render.
...
* `connectionState`: The state of WebSocket connection, used to switch between the member count text and the network status information.
...
* `leadingContent`: Represents the leading content part of the header. By default shows a back button.
...
* `centerContent`: Represents the core and center part of the header. By default shows information about the channel.
...
* `trailingContent`: Represents the content at the end of the header. By default shows the channel avatar.
