...
* **ReactionIconFactory**: Used to create a reaction icon for the given reaction type. You can use the default factory that supports predefined set of reactions, or provide a custom one.
...
You can easily customize how the images are loaded by overriding `ChatTheme.imageLoader` with your own implementation of Coil `ImageLoader`.
