...
The `ChannelList` component is being updated based on the data from two sources:
...
:::note
The `ChatEventHandler` is being provided by `ChatEventHandlerFactory` which gives you access to the visible channels map. By using the map, you can skip unwanted channel list updates.
:::
...
Alternatively, you can pass custom handler to `ChatClient::queryChannelsAsState` function, if you are not using UI SDKs.
...
- First one displaying `public` channels
- Second one for displaying `private` ones
Both channel types require a user to be a member to interact with the channel

As mentioned above, by default, channel-related events are not filtered, so if you would use the default `ChatEventHandler`, both lists would be updated regardless of the event's channel type.

You can fix that by providing two different event handlers.

The first one is going to handle `public` channels updates:
