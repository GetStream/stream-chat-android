...
In some cases, users may want to add their own database/ORM to Stream Android SDK instead of using the default one. This can be useful to add side effects when a certain data is used to control the flow of information in a specific way, use it to debug the SDK or support an existing database that already exists in the app without adding a new one.
