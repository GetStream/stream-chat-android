...
We store only the basic information about the sorting and allow the SDK to convert between entity and model classes. The SDK provides some methods to make it easiert, like `SortDirection.fromNumber(int)`.
...
![filter_graph](../../assets/filter_graph.png)

Now that we defined the structure of the composition of filters, we need to define as code how building block. 
...
Every filter must have a type and the properties `field` and `value` should be populated accordingly with its type.
...
By converting `Filters` to `FilterNode` we have an structure that contains only one class and then we can create a serialization adapter for this class making the process of serializing and deserializing `Filters` easy.
...
**Filter nodes** are nodes that realize and operation like compating if a field is equal to a value, if a field is bigger than a value, etc. 
...
Now that defiened the function `FilterObject.toFilterNode` we already know how to convert any `FilterObject` into a composition of `FilterNodes`. Now we can serialize our nodes.
...
Each node should be created accondingly if it is a node made of **other nodes**, a composite node, a node with many values like `IN` and `NOT_INT` or a simple node like **EQUALS**. 


## Deserializing FilterNode
In the oposite way, it is possible to create a `FilterNode` from a `String`. 
