...
In some cases, uses may want to add their own database/ORM to Stream Android SDK instead of using the default one. This can be useful to add side effects when a certain data is used to controll the flow of information in a spefic way, use it to debug the SDK or support an existing database that already exists in the app without adding a new one. 
