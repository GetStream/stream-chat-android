...
In most cases we strongly recommend using offline support as it significantly improves the user experience, however, certain types of applications (e.g. focused on livestreaming) don't require it. <br/>
In order to not necessarily increase the SDK size by including the database dependency, the `OfflinePlugin` is published as a separate library and needs to be configured as described on the [Getting Started](../../01-basics/04-getting-started.mdx#adding-offline-support) page:
...
The next step is to connect the `OfflinePlugin` with the `ChatClient`:
...
The `OfflinePlugin` works as a persistence layer - its responsibility is to save data into the offline storage, so it can be accessed later even if the application was killed.
...
You can learn more about the `StatePlugin` [here](./03-state-plugin.mdx).

### Supported Operations

Besides storing lists of channels, messages or reactions, the offline library allows you to perform the following actions without the Internet connection:
- Creating a channel
...
The offline storage is persisted even if the application is killed.
You can clear it during user disconnection by passing `flushPersistence = true` flag:
...
This will wipe all user's data from the storage.
