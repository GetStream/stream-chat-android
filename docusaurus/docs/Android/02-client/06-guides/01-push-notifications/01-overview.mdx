...
:::note
Push notifications are enabled by default on the client-side.
Keep in mind that our SDK creates a `NotificationChannel` during its initialization if push notifications are enabled.
You can disable them by creating custom `NotificationConfig` with `pushNotificationsEnabled` set to `false`.
:::
...
There are three ways to customize push notifications:
...
:::info
The SDK supports multiple languages. Make sure to override resources in all languages used in your app. Refer to [Adding Localization](../../../03-ui/05-guides/04-custom-translations.mdx) guide for more details.
:::
