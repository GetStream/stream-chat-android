...
You will need to add the list of `PushDeviceGenerator` you want to use:
```kotlin {1-5,8}
...
* `NotificationHandlerFactory`: Use styles we provide but customizing which Intent is triggered when clicking on a notification.
* `NotificationHandler`: Full customization how notification are shown and dismissed

### Using our `NotificationHandlerFactory`

Our `NotificationHandlerFactory` provide a way to create a default implementation of `NotificationHandler` based on the Android API Level and allows you to customize which Intent is triggered when the notification is clicked by the user.
This function allows you to provide a custom `Intent` that can point to any `Activity` in your app. You can also add custom data to the `Intent` if you need it.
...
The SDK also allows you to generate the notification UI that users see after they receive a push notification and have full control over the notification that is shown.
All you need to do is implement `NotificationHandler` interface and show your own notification. Because notifications are handled by this class, in order to dismiss notification when a channel is visited or when the user logout from the app, you will need to implement methods for dismissing notifications
