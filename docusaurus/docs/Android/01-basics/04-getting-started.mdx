...
Our UI Components rely on reading state in order to render the UI. Information such as the currently active user, list of muted users, online status, etc. is saved as state inside various state holders (e.g. `GlobalState`, and others). State is managed by `StatePlugin` and adding it to `ChatClient` is mandatory if you want to use our UI Components.
...
A great chat solution should be capable of handling the device losing network connection. `OfflinePlugin` adds a layer of persistence which enables storing data and user interactions while the device is offline. With the help of this plugin users will be able to access previously cached information such as channels and messages and have the ability to do other actions such as send messages and reactions while offline.
