...
The state library is a plugin that gives you the possibility to reactively observe data through `StateFlows`.
Moreover, the plugin adds a sync feature, that for example is responsible for resending messages, reactions, etc, when the user performed actions without an Internet connection.

:::note
The state plugin doesn't add a persistence layer.<br/> Consider using it together with `Offline Plugin`.
:::

<!-- TODO update link once State documentation is ready -->
This plugin exposes easy-to-use `StateFlow` objects for reading channels, messages, reads, typing, members, watchers and more. See [State](../02-client/06-guides/04-offline-support.mdx) for more details.

If you want to build custom UI for Stream Chat, you can build it on top of the state library.
...
This library is built on top of the state library, and offers the quickest integration of Stream Chat into an Android application. It also has a variety of [theming](../03-ui/02-general-customization/01-theming.mdx) options to make it fit your app's needs.
