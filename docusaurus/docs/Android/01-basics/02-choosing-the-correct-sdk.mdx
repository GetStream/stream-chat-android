...
:::note
If you're just interested in a TL;DR / our final overview of the SDKs, [scroll to the bottom](#the-right-sdk-for-you).
:::
...
- **Main building blocks**: Large elements crucial to a chat experience such as lists, headers and the message composer.
...
Notice how Compose Compose generates fewer new files and has fewer lines of code changed, due to a lack of XML layouts?
...
However, Jetpack Compose has been out for less than a year - it's a much more recent technology and that as such, it will keep
receiving performance improvements in the future. It already outperforms our UI Components SDK in some aspects, but if you choose our Compose SDK, you should expect a performance
