...
To use state library in your application, add the following dependency:
...
We ship additional artifact for Markdown support that can be used together with UI Components library:
 ...
 For more information see [UI Components Configuration](../03-ui/02-general-customization/02-chatui.mdx#markdown) guide.

### Push Notifications

We ship multiple artifacts to easily integrate Stream Chat with third party push notification providers. See the [Push Notification page](../02-client/06-guides/01-push-notifications/01-overview.mdx) for more details.
