...
* [`SearchInputView`](../utility-components/search-view.mdx): An input field to search for messages that contain specific text.
* [`SearchResultListView`](../utility-components/search-view.mdx): Displays a list of search result items.
...
:::note
Fragments and Activities representing self-contained screens can be styled using the options described in the [theming](02-ui/02-theming.mdx) guide.
:::
