...
To use the [UI Components](../ui/overview.mdx) in your application, add the following dependency:
...
To use the [Compose UI Components](../compose/overview.mdx) instead, add the following dependency:
...
We ship an additional artifact for Markdown support which can be used together with the UI Components library:
 ...
 For more information see [UI Components Configuration](../ui/general-customization/chatui.mdx#markdown) guide.

### Push Notifications

We ship multiple artifacts to easily integrate Stream Chat with third party push notification providers. See the [Push Notification page](../client/guides/push-notifications/01-overview.mdx) for more details.
