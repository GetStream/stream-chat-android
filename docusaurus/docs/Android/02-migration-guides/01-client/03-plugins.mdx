...
In addition to that, parameters that were previously included in the `Config` have been removed from th **SDK** as well:
...
:::note
The purpose of the `Offline plugin` was narrowed down to providing offline support, i.e. persistence layer, for the chat functionality.
:::
