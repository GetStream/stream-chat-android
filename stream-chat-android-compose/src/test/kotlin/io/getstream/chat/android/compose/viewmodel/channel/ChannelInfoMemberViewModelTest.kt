...
internal class ChannelInfoMemberViewModelTest {

    @Test
    fun `state should reflect controller state`() = runTest {
        val stateFlow = MutableStateFlow<ChannelInfoMemberViewState>(ChannelInfoMemberViewState.Loading)
        val sut = Fixture()
            .givenControllerStateFlow(stateFlow)
            .get()

        sut.state.test {
            assertEquals(ChannelInfoMemberViewState.Loading, awaitItem())
        }
    }

    @Test
    fun `events should reflect controller events`() = runTest {
        val eventsFlow = MutableSharedFlow<ChannelInfoMemberViewEvent>()
        val sut = Fixture()
            .givenControllerEventsFlow(eventsFlow)
            .get()

        sut.events.test {
            val event = ChannelInfoMemberViewEvent.RemoveMember(member = randomMember())
    ...
    fun `onViewAction should delegate to controller`() = runTest {
        ...
        val action = ChannelInfoMemberViewAction.BanMemberClick
    ...
    private class Fixture {
        private val mockController: ChannelInfoMemberViewController = mock()

        fun givenControllerStateFlow(stateFlow: MutableStateFlow<ChannelInfoMemberViewState>) = apply {
            whenever(mockController.state) doReturn stateFlow
        }

        fun givenControllerEventsFlow(eventsFlow: MutableSharedFlow<ChannelInfoMemberViewEvent>) = apply {
            whenever(mockController.events) doReturn eventsFlow
        }

        fun verifyControllerOnViewAction(action: ChannelInfoMemberViewAction) = apply {
            verify(mockController).onViewAction(action)
        }

        fun get() = ChannelInfoMemberViewModel(
            cid = randomCID(),
