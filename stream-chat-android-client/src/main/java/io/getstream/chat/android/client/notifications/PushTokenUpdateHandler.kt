...
internal class PushTokenUpdateHandler(context: Context) {
    private val logger = StreamLog.getLogger("Chat:Notifications-UH")
    ...
    suspend fun updateDeviceIfNecessary(device: Device) {
        ...
        updateDebouncer.submitSuspendable {
            ...
            if (result.isSuccess) {
                ...
                logger.e { "[updateDeviceIfNecessary] failed registering device ${result.error().message}" }
            }
        }
    }
    ...
    private suspend fun removeStoredDeviceInternal(): Boolean {
        ...
        logger.e { "[removeStoredDeviceInternal] failed: ${result.error()}" }
        return false
    }
