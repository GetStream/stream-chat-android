...
internal class IsoDateAdapter {
    ...
    // Guards the non-thread-safe SimpleDateFormat instances used on the API < 26 fallback paths.
    // ITU (API 26+) and LruCache are already thread-safe, so they run without this lock.
    private val legacyFormatterLock = Any()

    @ToJson
    internal fun toJson(value: Date): String {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ITU.formatUtcMilli(value.toInstant().atOffset(ZoneOffset.UTC))
        } else {
            synchronized(legacyFormatterLock) { withMillis.format(value) }
        }
    }
    ...
    internal fun fromJson(value: String): Date? {
        ...
        val parsed = try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                synchronized(legacyFormatterLock) { withMillis.parse(value) }
            }
        } catch (_: Throwable) {
            try {
                synchronized(legacyFormatterLock) { withoutMillis.parse(value) }
            } catch (_: Throwable) {
                null
            }
        }
