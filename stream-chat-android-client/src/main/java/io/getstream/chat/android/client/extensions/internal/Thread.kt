...
public fun Thread.upsertReply(reply: Message): Thread {
    ...
    // Update participant recency only when a new reply is inserted (not on edits/updates).
    ...
    val participantCount = if (isInsert) threadParticipants.size else this.participantCount
...
public fun Thread.applyThreadUpdatedEventChanges(threadInfo: ThreadInfo): Thread {
    ...
    return this.copy(
        title = threadInfo.title,
        updatedAt = threadInfo.updatedAt,
        extraData = threadInfo.extraData,
    )
}
