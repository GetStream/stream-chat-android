...
internal class MoshiChatApiTest {
    ...
    fun testRejectInvite(call: RetrofitCall<ChannelResponse>, expected: KClass<*>) = runTest {
        ...
        // then
        val expectedBody = GeneratedUpdateChannelRequest(rejectInvite = true)
        result `should be instance of` expected
    ...
    fun testAcceptInvite(call: RetrofitCall<ChannelResponse>, expected: KClass<*>) = runTest {
        ...
        // then
        val expectedBody = GeneratedUpdateChannelRequest(
            acceptInvite = true,
            message = GeneratedMessageRequest(text = message),
        )
        result `should be instance of` expected
    ...
    fun testAddMembers(call: RetrofitCall<ChannelResponse>, expected: KClass<*>) = runTest {
        ...
        val skipPush = randomBoolean()
        val result =
            sut.addMembers(channelType, channelId, members, systemMessage, hideHistory, hideHistoryBefore, skipPush)
                .await()
        // then
        result `should be instance of` expected
    ...
    fun testPartialUpdateThread(call: RetrofitCall<UpdateThreadPartialResponse>, expected: KClass<*>) = runTest {
        ...
        // then
        val expectedBody = PartialUpdateThreadRequest(set = set, unset = unset)
        result `should be instance of` expected
    ...
    fun testDeletePollOption(call: RetrofitCall<Response>, expected: KClass<*>) = runTest {
        ...
        val pollId = randomString()
        val optionId = randomString()
        val result = sut.deletePollOption(pollId, optionId).await()
        // then
        result `should be instance of` expected
        verify(api, times(1)).deletePollOption(pollId, optionId)
    }
    ...
    fun testGetPoll(call: RetrofitCall<PollResponse>, expected: KClass<*>) = runTest {
        // given
        val api = mock<PollsApi>()
        whenever(api.getPoll(any())).doReturn(call)
        val sut = Fixture()
            .withPollsApi(api)
            .get()
        // when
        val pollId = randomString()
        val result = sut.getPoll(pollId).await()
        // then
        result `should be instance of` expected
        verify(api, times(1)).getPoll(pollId)
    }
    ...
    @ParameterizedTest
    @MethodSource("io.getstream.chat.android.client.api2.MoshiChatApiTestArguments#queryPollsInput")
    fun testQueryPolls(call: RetrofitCall<QueryPollsResponse>, expected: KClass<*>) = runTest {
        // given
        val api = mock<PollsApi>()
        whenever(api.queryPolls(any())).doReturn(call)
        val sut = Fixture()
            .withPollsApi(api)
            .get()
        // when
    ...
    fun testQueryPollVotes(call: RetrofitCall<PollVotesResponse>, expected: KClass<*>) = runTest {
        ...
        val api = mock<PollsApi>()
        val pollId = randomString()
        whenever(api.queryPollVotes(any(), any())).doReturn(call)
        val sut = Fixture()
            .withPollsApi(api)
            .get()
        // when
