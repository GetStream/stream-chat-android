...
import io.getstream.chat.android.client.offline.repository.domain.reaction.toEntity
...
internal class ReactionRepositoryTest {
    ...
    fun `When dao returns reactions with syncNeeded == true they should contain current user data`() = runBlockingTest {
        whenever(reactionDao.selectSyncNeeded()).thenReturn(
            listOf(
                randomReaction {
    ...
    fun `When dao returns reactions for specific messageId they should contain user data`() = runBlockingTest {
        ...
        whenever(reactionDao.selectUserReactionsToMessage(messageId, currentUser.id)).thenReturn(
            listOf(randomReaction { syncStatus = SyncStatus.SYNC_NEEDED }.toEntity())
        )
