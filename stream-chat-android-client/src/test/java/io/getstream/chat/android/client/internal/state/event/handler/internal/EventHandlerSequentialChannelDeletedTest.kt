...
internal class EventHandlerSequentialChannelDeletedTest {
    ...
    @MethodSource("channelDeletions")
    fun `channel deletion is recorded on the channel state, which keeps its messages`(
        name: String,
        useLegacyChannelState: Boolean,
        deletion: CidEvent,
    ) = runTest {
        val fixture = Fixture(useLegacyChannelState)
        val channelState = fixture.withActiveChannel()
        ...
        eventHandler.handleEvents(randomNewMessageEvent(cid = CID, message = message))

        eventHandler.handleEvents(deletion)

        channelState.channelData.value.deletedAt `should be equal to` deletion.createdAt
        channelState.messages.value.map(Message::id) `should be equal to` listOf(message.id)
        fixture.isActiveChannel() `should be equal to` false
    }

    private class Fixture(useLegacyChannelState: Boolean) {
        ...
        private val stateRegistry = StateRegistry(
            ...
            useLegacyChannelState = useLegacyChannelState,
        )
        private val logicRegistry = LogicRegistry(
            ...
            now = { 0L },
            useLegacyChannelLogic = useLegacyChannelState,
            isLocalUnreadCountEnabled = false,
        )

        /** Watches the channel the way an open chat screen does, and returns the state it observes. */
        fun withActiveChannel(): ChannelState {
            logicRegistry.channel(CHANNEL_TYPE, CHANNEL_ID)
            return stateRegistry.channel(CHANNEL_TYPE, CHANNEL_ID)
        }

        fun isActiveChannel() = logicRegistry.isActiveChannel(CHANNEL_TYPE, CHANNEL_ID)
    ...
    companion object {
        ...
        @JvmStatic
        fun channelDeletions() = listOf(true, false).flatMap { useLegacyChannelState ->
            val state = if (useLegacyChannelState) "legacy state" else "state"
            listOf(
                Arguments.of(
                    "${EventType.CHANNEL_DELETED}, $state",
                    useLegacyChannelState,
                    randomChannelDeletedEvent(cid = CID, channelType = CHANNEL_TYPE, channelId = CHANNEL_ID),
                ),
                Arguments.of(
                    "${EventType.NOTIFICATION_CHANNEL_DELETED}, $state",
                    useLegacyChannelState,
